...
def _internal_api_base() -> str:
    return _clean_env(os.getenv("INTERNAL_API_BASE", "http://127.0.0.1:8080"), "http://127.0.0.1:8080").rstrip("/")
...
def _apply_sql_patch_direct(sql_patch: str, table_name: str) -> Dict[str, Any]:
    from ...db import ENGINE  # local import to avoid circular import during module load

    if ENGINE is None:
        return {"ok": False, "reason": "engine_unavailable", "table": table_name}

    try:
        statements = [stmt.strip() for stmt in (sql_patch or "").split(";") if stmt.strip()]
        with ENGINE.begin() as conn:
            for stmt in statements:
                conn.execute(text(stmt))
        print(f"AUTO_SCHEMA_HOTFIX_APPLIED table={table_name}")
        return {"ok": True, "table": table_name, "applied_statements": len(statements)}
...
def maybe_trigger_schema_patch(error_text: str, path: str = "runtime_auto") -> Dict[str, Any]:
...
def evolution_trigger_health():
    return {
        ...
        "auto_schema_patch_create_pr": _env_flag("AUTO_SCHEMA_PATCH_CREATE_PR", False),
